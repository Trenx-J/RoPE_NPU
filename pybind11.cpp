...
namespace RoPE {
    at::Tensor RoPE1D(at::Tensor &x, at::Tensor &c, at::Tensor &s, uint32_t &t)
    {
        ...
        const_cast<void *>(result.storage().data()), 
        totalLength, input_dim, tilingLength, datatype);
        return result;
        
    }

    at::Tensor RoPE3D(at::Tensor &x, at::Tensor &c, at::Tensor &s, uint32_t &t)
    {
        auto acl_stream = c10_npu::getCurrentNPUStream().stream(false);
        uint32_t blockDim = 48;
        uint32_t totalLength = 1;
        uint32_t tilingLength = t;
        uint32_t input_dim = x.sizes()[3]/ 3;
        for (uint32_t size : x.sizes()) {
            totalLength *= size;
        }    
        if( input_dim == 96)
...
PYBIND11_MODULE(rope_custom, m)
{
    ...
    m.def("rope1d", &RoPE::RoPE1D, "");
    m.def("rope3d", &RoPE::RoPE3D, "");
}
